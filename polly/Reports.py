class Reports:
    ...
    def generate_report(self, repo_key: str, dataset_id: str, workspace_id: int, workspace_path="") -> None:
       ...
       Args:
           repo_key (str/int): repo_key(repo_name/repo_id) for which the report is to be generated
    ...
    def link_report(
       ...
       Org admins can now link a report (html or pdf file format) present in a workspace with the specified datasets in an OmixAtlas.
       Once a report is linked to a dataset in OmixAtlas, it can be fetched both from front-end and polly-python.
 
       Args:
           repo_key (str/int): repo_key(repo_name/repo_id) of the repository to be linked
           ...
           Access_key (str): access_key(private or public) depending upon the link access type to be generated. If public, then anyone with a Polly account with the link will be able to see the report. If private, then only the individuals who have access to the workspace where reports is stored will be able to see them.
    ...
    def fetch_linked_reports(self, repo_key: str, dataset_id: str) -> pd.DataFrame:
       ...
       Args:
           repo_key (str/int): repo_key(repo_name/repo_id) of the repository for which to fetch the report
    ...
    def delete_linked_report(self, repo_key: str, dataset_id: str, report_id: str) -> None:
       ...
       Arguments:
           repo_key (str/int): repo_key(repo_name/repo_id) of the repository which is linked.
